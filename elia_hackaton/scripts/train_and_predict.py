...
from elia_hackaton.core.plot import plot_training_curves
...
for filename in os.listdir(RESULTS_DIR):
    if filename.endswith(".csv"):
        ...
        # Target is hotspot temperature instead of general temperature
        y_true = equipment_df['hotspotTemperature'].values.reshape(-1, 1)
...
for filename in os.listdir(RESULTS_DIR):
    if filename.endswith(".csv"):
        ...
        try:
            ...
            # Plot actual hotspot temperatures for the entire dataset
            plt.plot(plot_df['dateTime'], plot_df['hotspotTemperature'], label='Actual Hotspot', color='black')
            
            # Plot white box predictions
            plt.plot(plot_df['dateTime'], plot_df['Temp Pred'], label='White Box Hotspot', color='blue', linestyle='--')
            ...
            plt.plot(plot_df['dateTime'], full_pinn_preds, label='PINN Hotspot', color='red')
            ...
            if sum(test_mask) > 0:
                test_actual = plot_df.loc[test_mask, 'hotspotTemperature'].values
            ...
            plt.title(f'Hotspot Temperature Prediction Comparison - {equipment_name}')
            plt.xlabel('Date')
            plt.ylabel('Hotspot Temperature')
            plt.grid(True, alpha=0.3)
            plt.legend()
            plt.tight_layout()
            
            # Save the plot
            plt.savefig( IMAGES_DIR / f'prediction_plot_{equipment_name}.png')
            plt.close()
            ...
            plt.title(f'Hotspot Temperature Prediction Comparison - {equipment_name} (PINN model unavailable)')
            plt.xlabel('Date')
            plt.ylabel('Hotspot Temperature')
            plt.grid(True, alpha=0.3)
            plt.legend()
        ...
        # Data is separated into smaller chunks to not exceed API upload limits
        chunk_size = 2950
        chunks = split_dataframe(equipment_df_test, chunk_size)

        # Comment out the API upload or replace with your actual endpoint
        upload_path = 'prediction/Hotspot'  # Changed from 'Temperature' to 'Hotspot'
...
print("\nHotspot temperature prediction generation and export completed.")
end_time = time.time()
