...
class SubstationManager:
    ...
    def analyze_transformer_capacity(self):
        ...
        for transformer_id, transformer in self.transformers.items():
            ...
            if high_load_mask.sum() > 0:
                high_load_temp_ratio = data.loc[high_load_mask, 'temperature'].mean() / data.loc[high_load_mask, 'K'].mean()
            else:
    ...
    def create_reinforcement_report_by_city(self):
        ...
        if priority_data:
            ...
            metrics_to_plot = ['priority_score', 'load_factor', 'load_headroom', 'max_temperature', 'high_temp_percentage']
            
            # Normalize the data for better visualization
            for col in metrics_to_plot:
                if col != 'city':
                    city_metrics[col + '_norm'] = (city_metrics[col] - city_metrics[col].min()) / (city_metrics[col].max() - city_metrics[col].min())
